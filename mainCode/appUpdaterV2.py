...
def appUpdater(root, photo,curr_version, curr_location, curr_directory, currFilename):
    ...
    try:
        ...
        if my_container.get_blob_client(f"{currFilename}_{curr_version}.exe").exists():
            ...
            labelFrame = tk.Frame(top, bg= "#9BC2E6")
            labelFrame.grid(row=1, column=1, columnspan=3,rowspan=3)

            updateLabel = ttk.Button(labelFrame, text="App is Updating, Please Wait...", anchor="center",background = "#9BC2E6",)
            updateLabel.grid(row=0, column=0, sticky="nsew")
