...
#EAGS_Quotation
def eagsQuotationuploader(conn,df,latest_revised_quote):
    try:
...
def getLatestQuote(conn,curr_quoteNo,previous_quote_number):
    try:
        ...
        query = f"SELECT QUOTENO FROM {DATABASE}.{SCHEMA}.{EAGS_QUOTATION_TABLE} WHERE INSERT_DATE IS NOT NULL AND REV_CHECKER = 1 AND QUOTENO like '%{cx_init_name_previous}%' ORDER BY QUOTENO desc LIMIT 1"
        ...
        if len(raw_data) and "_" in raw_data.iloc[0,0]:
            ...
            #---------Updating previous rev checker to 0 ----------
            # rev_query = f'''UPDATE {DATABASE}.{SCHEMA}.{EAGS_QUOTATION_TABLE} set REV_CHECKER = 0 Where QUOTENO = '{raw_data}';'''
            # cur.execute(rev_query)
